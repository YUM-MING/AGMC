...
const CONTENT_SUGGESTIONS = [
  { name: '도트 판타지', prompt: '고전 16비트 JRPG 스타일의 도트 그래픽으로 판타지 세계관을 묘사해줘. 주인공, 몬스터, 배경 에셋에 대한 상세한 외형 지시를 포함해줘.' },
  { name: '사이버펑크', prompt: '네온사인이 가득한 어두운 사이버펑크 도시를 배경으로 해줘. 메탈릭한 질감과 홀로그램 빛이 강조된 에셋 디자인을 상세히 묘사해줘.' },
  { name: '포스트 아포칼립스', prompt: '황폐화된 지구를 배경으로 한 생존 게임 느낌의 비주얼을 기획해줘. 녹슨 고철, 방독면, 돌연변이 식물 등의 에셋 묘사를 포함해줘.' },
  { name: '캐주얼 카툰', prompt: '동물의 숲이나 스타듀밸리 같은 부드럽고 따뜻한 파스텔톤의 캐주얼 카툰 스타일로 비주얼을 기획해줘. 귀여운 캐릭터 중심의 에셋 묘사가 중요해.' }
];

const OPS_SUGGESTIONS = [
  { name: '초반 튜토리얼 완화', prompt: '유저들의 초반 이탈률을 낮추기 위해 튜토리얼 구간의 난이도를 완화하고 보상을 강화하는 운영안을 기획해줘.' },
  { name: '엔드 콘텐츠 밸런싱', prompt: '고인물 유저들이 지루해하지 않도록 엔드 콘텐츠의 난이도를 상향하고 명예 보상을 추가하는 밸런싱 아이디어를 제안해줘.' },
  { name: '이벤트 기획', prompt: '주말 동안 유저 접속률을 높일 수 있는 특별한 인게임 버프나 미니 이벤트 기획안을 작성해줘.' }
];

const ANALYTICS_SUGGESTIONS = [
  { name: '초반 이탈 구간 분석', prompt: '현재 프로토타입에서 유저들이 가장 많이 이탈할 것으로 예상되는 구간(허들)을 예측하고 그 원인을 분석해줘.' },
  { name: '재미 요소 도출', prompt: '이 게임의 메인 루프에서 유저가 가장 큰 도파민(재미)을 느낄 수 있는 핵심 포인트가 무엇인지 분석해줘.' },
  { name: '플레이 타임 증대 방안', prompt: '유저들의 평균 플레이 타임을 늘리기 위해 어떤 시스템적 보완이 필요한지 데이터 관점에서 통찰을 제시해줘.' }
];
