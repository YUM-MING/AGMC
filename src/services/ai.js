...
export const requestImageGeneration = async (prompt, numImages = 1, category = "캐릭터") => {
  ...
  if (category === "배경") {
    ...
    categoryRule = "캐릭터 아트입니다. 인물을 클래식 온라인 게임 캐릭터로 재해석. 도트 그래픽 기반의 치비 아바타. 옛날 한국 MMORPG 캐릭터 선택창에 나올 법한 스타일. 사용자가 말한 헤어스타일, 머리색, 의상, 액세서리, 표정, 분위기를 유지하고, 이를 단순화된 도트 디자인으로 표현. 전체 크기는 작은 스프라이트 느낌. 동그란 얼굴, 큰 머리, 짧은 팔다리. 단순한 눈과 작은 입으로 귀엽게 표현. 선명한 픽셀 윤곽선, 깨끗한 색 분리, 부드러운 파스텔 명암 처리. 게임 아이콘이나 아바타 스티커처럼 완성. 누끼 따고(흰 배경), 전신, 정면. 실제 인물의 특징은 유지하되, 복잡한 디테일은 클래식 게임 아바타 수준으로 단순화. Negative Prompt: 실사, 애니메이션 일러스트, 3D 렌더링, 과도한 디테일, 흐릿한 픽셀, 복잡한 배경, 추가 팔다리, 왜곡된 비율, 텍스트, 워터마크";
  }
